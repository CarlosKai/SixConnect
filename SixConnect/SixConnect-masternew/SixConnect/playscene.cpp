...
PlayScene::PlayScene(int index)
{
    ...
    if(index==1){
        ...
        countTime=new QLabel(this);
        countTime->setText("预备");
        countTime->show();
        countTime->move(990,200);
        QString strStyle = "QLabel{"
                           "font-family: \"Microsoft YaHei\";"
                           "font-size: 60px;"
                           "color: rgb(50, 50, 50, 180);"
                           "}";
        countTime->setStyleSheet(strStyle);
        countTime->resize(200,100);
    ...
    if(index==2){
        ...
        connect(stopBtn,&MyPushButton::clicked,this,[=](){
            ...
            QMessageBox::StandardButton btnValue = QMessageBox::information(this, "Waiting", " Game Stop!");
            // 重置游戏状态，否则容易死循环
    ...
    if(index==1){
        ...
        //悔棋按钮
        MyPushButton *withdrawBtn=new MyPushButton(":/res/withdraw2.png",":/res/withdraw.png");
        withdrawBtn->setParent(this);
        withdrawBtn->move(this->width()*0.97-withdrawBtn->width(),this->height()*0.75-withdrawBtn->height());

        connect(stopBtn,&MyPushButton::clicked,this,[=](){
            backSound->play();
            if(pTimer&&timeStop==0){
                pTimer->stop();
            }
            QMessageBox::StandardButton btnValue = QMessageBox::information(this, "Waiting", " Game Stop!");
            // 重置游戏状态，否则容易死循环
        ...
        connect(withdrawBtn,&MyPushButton::clicked,this,[=](){
            backSound->play();
            if(game->position-1>=0){
                game->map[game->lastRow[game->position-1]][game->lastCol[game->position-1]]=0;
                game->playerFlag=game->lastPlayer[game->position-1];
                (game->position)--;
            }
            else{
                QMessageBox::StandardButton btnValue = QMessageBox::information(this, "Waiting", "There is no chess!");
            }
...
{
    qDebug()<<game->lastRow<<"    "<<game->lastCol<<"    "<<game->playerFlag;
    // 人下棋，并且不能抢机器的棋
    if (game->choose== 1 )
    {
        chessOneByPerson();
...
void PlayScene::on_timer_timeout()
{
    if (value == 0)
    {
        if (game->gameStatus == PLAYING)
        {
            ...
            deadSound->play();
            QString str;
            if (game->lastPlayer[game->position-1] == 1)
                str = "白棋 玩家";
            else if (game->lastPlayer[game->position-1] == -1)
                str = "黑棋 玩家";
            else if(game->lastPlayer[game->position-1]==0)
                str="黑棋 玩家";
            QMessageBox::StandardButton btnValue = QMessageBox::information(this, "Game Over!", str + " Time out!");
    ...
    countTime->setText(QString::number(value));
    value--;
